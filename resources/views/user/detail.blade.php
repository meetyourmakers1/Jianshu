...
@section('content')
    <div class="col-sm-8">
        <blockquote>
            <p>
                <img src="" alt="" class="img-rounded" style="border-radius:500px; height: 40px"> {{$user->name}}
            </p>
            <footer>关注：{{$user->stars_count}}｜粉丝：{{$user->fans_count}}｜文章：{{$user->posts_count}}</footer>
            @include('user.button.button',['user' => $user])
        </blockquote>
    </div>
                ...
                <div class="tab-pane active" id="tab_1">
                    @foreach($posts as $post)
                        <div class="blog-post" style="margin-top: 30px">
                            ...
                            <p class=""><a href="/posts/detail/{{$post->id}}" >{{$post->title}}</a></p>
                            <p>{!! $post->content !!}</p>
                        </div>
                    @endforeach
                </div>
