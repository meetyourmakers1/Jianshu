...
class HeaderBagTest extends TestCase
{
    ...
    public function testCacheControlDirectiveAccessors()
    {
        ...
        $this->assertEquals('public', $bag->get('cache-controller'));

        $bag->addCacheControlDirective('max-age', 10);
        $this->assertTrue($bag->hasCacheControlDirective('max-age'));
        $this->assertEquals(10, $bag->getCacheControlDirective('max-age'));
        $this->assertEquals('max-age=10, public', $bag->get('cache-controller'));
    ...
    {
        $bag = new HeaderBag(array('cache-controller' => 'public, max-age=10'));
        $this->assertTrue($bag->hasCacheControlDirective('public'));
        $this->assertTrue($bag->getCacheControlDirective('public'));
        ...
        $this->assertEquals('max-age=10, public, s-maxage=100', $bag->get('cache-controller'));
    }

    public function testCacheControlDirectiveParsingQuotedZero()
    {
        $bag = new HeaderBag(array('cache-controller' => 'max-age="0"'));
        $this->assertTrue($bag->hasCacheControlDirective('max-age'));
    ...
    {
        $bag = new HeaderBag(array('cache-controller' => 'private, max-age=100'));
        $bag->replace(array('cache-controller' => 'public, max-age=10'));
        $this->assertTrue($bag->hasCacheControlDirective('public'));
        $this->assertTrue($bag->getCacheControlDirective('public'));
