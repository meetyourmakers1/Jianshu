...
class ResponseHeaderBagTest extends TestCase
{
    ...
    public function testCacheControlHeader()
    {
        ...
        $bag = new ResponseHeaderBag(array('cache-controller' => 'max-age=100'));
        $this->assertEquals('max-age=100, private', $bag->get('Cache-Control'));

        $bag = new ResponseHeaderBag(array('cache-controller' => 's-maxage=100'));
        $this->assertEquals('s-maxage=100', $bag->get('Cache-Control'));

        $bag = new ResponseHeaderBag(array('cache-controller' => 'private, max-age=100'));
        $this->assertEquals('max-age=100, private', $bag->get('Cache-Control'));

        $bag = new ResponseHeaderBag(array('cache-controller' => 'public, max-age=100'));
